...
function check() {
    ...
    if (!name || !classes || !facility || !complain || !izin || !drp) {
        return false;
    } else {
...
const alrtdrop = document.querySelector('.alrtDrpbtnText')
function drpbtn() {
    if (btnForm.textContent === "FASILITAS") {
        alrtdrop.classList.add("show");
        return false;
    } else {
        alrtdrop.classList.remove("show");
        return true;
    }
}
